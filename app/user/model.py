...
class UserInputBaseModel(BaseModel):
    ...
    is_status: Optional[int] = 0
    create_time: Optional[int] =  Field(default_factory=lambda: int(datetime.now().timestamp()))
    update_time: Optional[int] = Field(default_factory=lambda: int(datetime.now().timestamp()))
    ...
    is_login: Optional[int] = 0 # 0:未登录 1:已登录